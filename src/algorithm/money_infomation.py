...
class MoneyInformation(Information):
    ...
    def text_search(self,info_soup):
        '''读入html，进行清洗，分割，去噪
                    从ngrams中按初步提取候选数据，再进行一轮清洗。
                    从清洗后的数据中按关键字提取内容
        return： list of money , list of unit 
        '''   
        rough_text = self.get_clean_text(info_soup)
#         print(rough_text)
...
#         print('origin',amount)
        if not unit or len(unit)!=1:
            if not unit:
                ...
                print('单位不统一')
            return 
        amount,unit = self.clean_currency.get_target_list(amount,unit)
#         print('proc',amount)
#         print(unit)
        if not amount:
            print('没有合法金额')
...
#         auxiliary_key =re.compile('第[一1]')
        auxiliary_key= pattern.FIRST_KEY_PATTERN

...
class Clean(object):
    ...
    def get_target_list(self, target_list, valid_pattern, get_target_function):
        ...
        >>> ner = Ner()
        >>> c = Clean(pattern)
        >>> c.get_target_list(['B公司', 'A公司', 'B公司', '不是company'], pattern.KEY_COMPANY_PATTERN, ner.get_institution_from_string_checked_len)
        ['B公司', 'A公司']
        """
        if target_list is None:
            return None
        if not target_list:
            return None
        valid_list = list(filter(lambda x: self.is_valid(x, valid_pattern), target_list))
        invalid_list = list(filter(lambda x: not self.is_valid(x, valid_pattern), target_list))
        valid_list_from_invalid = self.get_target_from_invalid_list(invalid_list, valid_pattern, get_target_function)
    ...
    def get_target_from_invalid_list(self, invalid_list, valid_pattern, get_target_function):
        ...
        if valid_list_from_bad is not None and valid_list_from_bad:
            valid_list_from_bad = list(filter(lambda x: self.is_valid(x, valid_pattern), valid_list_from_bad))
        return valid_list_from_bad


    def is_valid(self, target, valid_pattern):
        if target is None:
            return False
        if not target:
            return False
...
if __name__ == '__main__':
    ...
    # from extraction import pattern
    # c = Clean(pattern)
    # print(c.remove_duplicates([1, 2, 2, 1, 3]))
    from extraction import pattern
    from algorithm.nlp_algorithm.ltp_algorithm import Ner
    import os
    os.chdir(os.pardir)
    ner = Ner()
    c = Clean(pattern)
    print(c.get_target_list(['这是个B公司', '这哈是A公司', 'NOnONOnB公司', '中标公司：成都科技有限公司'], pattern.KEY_COMPANY_PATTERN,
